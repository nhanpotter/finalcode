...
def word_embed_meta_data(documents, embedding_dim, model):
    ...
    model['12345678910'] = (model['1'] + model['2'] + model['3'] + model['4'] + model['5'] + model['6'] + model['7'] +
                            model['8'] + model['9'] + model['10']) / 10
...
def create_train_dev_set(tokenizer, answers_pair, feat, ans_grade, max_sequence_length, validation_split_ratio):
    ...
    feat_train = pd.DataFrame(feat_train_dict,
                              columns=['Len Answer', 'Len Ref Answer', 'Len Ref By Ans', 'Words Ans', 'Uniq Words Ans'])
    ...
    feat_val = pd.DataFrame(feat_val_dict,
                            columns=['Len Answer', 'Len Ref Answer', 'Len Ref By Ans', 'Words Ans', 'Uniq Words Ans'])
